...
class Client extends DispatcherBase {
  ...
  [kDispatch] (opts, handler) {
    ...
    bindQueuedAbort(opts, request, abort)
...
function bindQueuedAbort (opts, request, abort) {
  const bindAbort = opts[kRequestSignal]

  // Public APIs reuse their existing signal listener through this private
  // binding. Return even when it throws so a second listener is never added.
  if (typeof bindAbort === 'function') {
    try {
      bindAbort(abort)
    } catch (err) {
      abort(err)
  ...
  // Keep queued aborts working when a custom interceptor copies only string
  // options and drops the private symbol used by the public API layer.
  const { signal } = opts
  const isSignalLike = signal && (
